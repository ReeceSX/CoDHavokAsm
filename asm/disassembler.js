...
function dmpConstants(_, level, constants) {
	...
	constants.objs.forEach((obj) => {
		append(_, level, fmt("%s \t%s", obj.tname, !obj.value ? "" : obj.value.toString()));
	});
	level--;
...
function alignInstruction(instruction) {
	var buf = instruction;
	for (var i = 0; i < getLargestOPLen() - instruction.length; i ++)
		buf += " ";
	return buf;
}
...
function dmpMethod(_, level, method) {
	...
	level++;
	append(_, level, fmt("TYPE\t%i", method.debug.type));
	append(_, level, fmt("DATA\t%s", JSON.stringify(method.debug.data)));
	level--;
	append(_, level, "END_DBG");
	append(_, level, "");
	dmpConstants(_, level, method.constants);
	for (var i in method.closures) {
		var ahh = method.closures[i];
		append(_, level, "");
		append(_, level, fmt("MARK_CLOSURE %i", i));
